  ...
  def search
    ...
    if params[:q].present?
      ...
      if params[:q][:home_type_eq_any].present?
        ...
        session[:House] = session[:home_type_eq_any].include?("House")
        session[:Apartment] = session[:home_type_eq_any].include?("Apartment")
        session[:Studio] = session[:home_type_eq_any].include?("Studio")
      else
        session[:home_type_eq_any] = ""
        session[:House] = false
        session[:Apartment] = false
        session[:Studio] = false
      end
